...
impl ActionManager {
    ...
    pub fn perform(
        ...
        use ActionManagerError::{
            ActionFailedToPerform, ActionNotFound, LockedCamera, LockedScene,
        };
        ...
        if let Some(scene_locked_action_name) = &self.scene_lock {
            if action.locks_scene() && action_name.ne(scene_locked_action_name) {
                return Err(LockedScene(
                    action_name,
                    scene_locked_action_name.to_string(),
                ));
            }
        }
        ...
        if let Some(camera_locked_action_name) = &self.camera_lock {
            if action.locks_camera() && action_name.ne(camera_locked_action_name) {
                return Err(LockedCamera(
                    action_name,
                    camera_locked_action_name.to_string(),
                ));
            }
        }

        if action.locks_scene() {
            self.scene_lock = Some(action_name.clone());
        }
        if action.locks_camera() {
            self.camera_lock = Some(action_name.clone());
        }
        ...
        match action.perform(project, console) {
            Ok(changes) => {
                ...
                if num_changes > 0 {
                    ...
                    match last_change {
                        Change::Start => (),
                        Change::End | Change::StartEnd(_) | Change::Untracked(_) => {
                            if action.locks_scene() {
    ...
    pub fn undo(
        ...
        use ActionManagerError::{ActionFailedToPerform, NothingToUndo};
        use ActionNameOrChangeIndex::*;
                        ...
                        match (*action).perform(project, console) {
                            Ok(changes) => {
                                for change in changes {
                                    ...
                                    // and for-loop will only run for 1 iter
                                    new_change = change.as_untracked()?;
                                }
                            }
                            Err(action_error) => {
                                return Err(ActionFailedToPerform(
    ...
    pub fn redo(
        ...
        use ActionManagerError::{ActionFailedToPerform, NothingToRedo};
        use ActionNameOrChangeIndex::*;
...
#[derive(Debug)]
pub enum ActionNameOrChangeIndex {
    ActionName(String),
    ChangeIndex(usize),
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        match self {
            ...
            ActionFailedToPerform(parameter, action_error) => write!(
                ...
                "{}: {}",
                match parameter {
                    ActionNameOrChangeIndex::ActionName(action_name) =>
                        format!("action '{}' failed to perform", action_name),
            ...
            LockedScene(action_name, scene_locked_action_name) => write!(
                ...
                action_name, scene_locked_action_name,
            ),
            LockedCamera(action_name, camera_locked_action_name) => write!(
                ...
                action_name, camera_locked_action_name,
            ),
...
impl From<ChangeError> for ActionManagerError {
    fn from(item: ChangeError) -> ActionManagerError {
        ActionManagerError::ChangeError(item)
    }
}
