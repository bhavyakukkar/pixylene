...
:END:
**** TODO _CORE_ [0/5]
***** TODO make application safer by having indexes like layer number only be something like u8 or u16 (this is very important now) this does not let the approach of move_to_last_layer work, and is probably what's crashing CircularOutline make new types PInt with From impl for primitives
***** TODO each layer has a blendmode (like in gimp)
***** TODO actions to make/move/remove layers
***** TODO ActionManager needs to detect overwriting of undo-history and cascade all changes after the overwriting change
...
***** TODO all project fields that have constraints should be private, for eg., project.cursors must be private, however, project.focus can be public (according to the system now at least). at the moment ToggleCursorAtFocus is able to create a cursor outside of the scene which bugs out every single action
...
***** TODO RectangularOutline Action

**** TODO _EXPORTS_ [0/3]
***** TODO scaling of artwork while exporting (64x64 "x20" = 1280x1280)
***** TODO Png to/fro Scene: fill more color-type/bit-depth combos
***** TODO pixylene::export adds black background that preview uses, let exports have transparent bgs
...
*** Making It Enjoyable To Use [0%]
**** TODO restructuring: rename common to types and put layer mod in project mod (layer, camera, scene & palette all are composed children of Project)
