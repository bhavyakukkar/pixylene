...
pub const CANVAS_MISMATCH_TRUE: &str = "cannot make use of Indexed type where True type is required";
pub const CANVAS_MISMATCH_INDEXED: &str = "cannot make use of True type where Indexed type is required";

pub static BOXED_ERROR: fn(&str) -> Box<dyn std::error::Error + Send + Sync> = |s: &str|
    Box::from(s);
