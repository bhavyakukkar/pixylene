...
impl TealData for PCoord {
    fn add_methods<'lua, T: TealDataMethods<'lua, Self>>(methods: &mut T) {
        ...
        methods.document_type(
            "A positive coordinate type composed of two positive (1 or greater) \
        ...
        // p = PCoord()       -- ->(1,1)
        {
            ...
            methods.document(
                "Create & return a new PCoord with optional 'x' and 'y' coordinates \
                             that default to 1",
            );
            methods.add_meta_method(MetaMethod::Call, |_, _, a: CoordArgs| {
                ...
                match types::PCoord::new(a.x.unwrap_or(1), a.y.unwrap_or(1)) {
                    Ok(p) => Ok(PCoord(p)),
                    Err(()) => Err(ExternalError(Arc::from(boxed_error(&format!(
        ...
        //Lua metamethod '+' interface to PCoord::add
        {
            ...
            methods.document(
                "Return a PCoord composed of the sums of two PCoord's coordinates, \
            ...
            methods.add_meta_function(MetaMethod::Add, |_, a: PCoordAddArgs| {
                ...
                let x = a
                    ...
                    .0
                    .x()
                    .checked_add(a.second.0.x())
                    .ok_or(ExternalError(Arc::from(boxed_error(
                        "Addition of x coordinates of the two PCoord's has overflowed",
                    ))))?;
                let y = a
                    .first
                    .0
                    .y()
                    .checked_add(a.second.0.y())
                    .ok_or(ExternalError(Arc::from(boxed_error(
                        "Addition of y coordinates of the two PCoord's has overflowed",
                    ))))?;
    ...
    fn add_fields<'lua, F: tealr::mlu::TealDataFields<'lua, Self>>(fields: &mut F) {
        ...
        fields.add_field_method_set("x", |_, this, value| {
            ...
            this.0.set_x(value).map_err(|_| {
                ExternalError(Arc::from(boxed_error("Trying to set x to 0 for PCoord")))
            })
        });
        ...
        fields.add_field_method_set("y", |_, this, value| {
            ...
            this.0.set_y(value).map_err(|_| {
                ExternalError(Arc::from(boxed_error("Trying to set y to 0 for PCoord")))
            })
        });
    }
}
