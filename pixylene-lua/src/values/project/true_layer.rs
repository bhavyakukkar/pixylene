...
impl TealData for TrueLayer {
    fn add_methods<'lua, T: TealDataMethods<'lua, Self>>(methods: &mut T) {
        ...
        //Lua interface to Layer::merge
        {
            ...
            methods.add_function("merge", |_, a: TrueLayerMergeArgs| {
                let top = a.top.0.do_imt::<_, _, CanvasMismatch<ContextExpired<
                    project::Layer
                >>>
                ...
                let bottom = a.bottom.0.do_imt::<_, _, CanvasMismatch<ContextExpired<
                    project::Layer
                >>>
                    (|layer| Ok(Ok(layer.clone())))
                    (|pixylene, index| pixylene.project.canvas.layers.to_true()
    ...
    fn add_fields<'lua, F: tealr::mlu::TealDataFields<'lua, Self>>(fields: &mut F) {
        ...
        fields.add_field_method_set("scene", |_, this, scene: TrueScene| {
            ...
            >>>
                (|scene| Ok(Ok(scene.clone())))
                (|pixylene, index| pixylene.borrow().project.canvas.layers.to_true()
                    .map(|layers| layers.get_layer(*index)
                         .map(|layer| layer.scene.clone())
                         .map_err(|_| ())))
                .map_err(|_| ExternalError(Arc::from(BOXED_ERROR(CANVAS_MISMATCH_TRUE))))?
            ...
            this.0.do_mut::<_, _, CanvasMismatch<ContextExpired<()>>>
                ...
                (|mut pixylene, index| pixylene.borrow_mut().project.canvas.layers.to_true_mut()
